...
2. **Review.** The lead gets the change and must delegate to all three specialists in one turn. Each specialist returns a JSON array of findings. Specialists run in the foreground, and a hook refuses the lead's answer while any specialist it started is still running, so a late report can never land after the answer and leave the run without one.
